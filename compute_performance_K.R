compute_performance_K <- function(adata, d = d, id = id, list_all = list_all, iter = iter, k = k_values[k], k_ratio = k_ratio[k], k_ideal = k_ideal){
  
  #Single-cell Spearman correlation
  spear_all = cor.test(x = adata$stemFinder, y = adata$Ground_truth, method = "spearman", exact = F)$estimate
  ...
  auc = auc_probability(category, prediction.sF)
  ...
  spear_pheno = cor.test(x = meanpotency$sF_potency, y = meanpotency$ground_truth)$estimate
  ...
  list_all$kideal[d] = k_ideal
  list_all$k[d] = k
  list_all$kratio[d] = k_ratio
  
  list_all$spear_all_sF[d] = spear_all
  list_all$auc_sF[d] = auc
  list_all$spear_pheno_sF[d] = spear_pheno
  list_all$pct.recov_sF[d] = pct.recov
